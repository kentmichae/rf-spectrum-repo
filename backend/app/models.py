...
class User(Base):
    ...
    region_id: Optional[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("regions.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class Equipment(Base):
    ...
    firmware_version: Optional[str] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class Observation(Base):
    ...
    bandwidth: Optional[float] = mapped_column(Float, nullable=True)
    modulation_type: Optional[str] = mapped_column(Text, nullable=True)
    signal_strength: Optional[float] = mapped_column(Float, nullable=True)
    ...
    equipment_id: Optional[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("equipment.id"), nullable=True)
    technician_id: Optional[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
...
class AuditTrail(Base):
    ...
    changed_by: Optional[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    change_timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    old_value: Optional[dict] = mapped_column(JSONB, nullable=True)
    new_value: Optional[dict] = mapped_column(JSONB, nullable=True)
